...
.PHONY: run-logs
run-logs: $(IMAGE_NAME).iso
	qemu-system-x86_64 -M q35 -m 2G -cdrom $(IMAGE_NAME).iso -boot d -serial mon:stdio -d int,cpu_reset -D qemu.log
